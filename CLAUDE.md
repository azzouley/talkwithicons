...
**Open item found during this cleanup, not fixed (out of scope):** the Vapi phone-number record for the `sittingbull` routing key (id `592fda30-7a31-49e7-b793-eb1f569a7d9c`, number `+14632886610`, still labeled "Friday") still has `assistantId` set to Casanova's old assistant (`101718d5-fc95-462c-8ca8-a966aefed7b4`), not Arthur Vance's (`bc8ba6dd-a90f-41cc-aa08-ca208107f864`), even though CLAUDE.md documents the Casanova → Vance swap as complete (2026-09-07). Outbound calls likely aren't affected (start-call-basic.js passes `assistantId` explicitly per call), but if that number is ever dialed inbound, it would currently reach the retired Casanova assistant, not Vance. Same class of bug as the Twain and La Llorona incidents below — flagged, not patched.
