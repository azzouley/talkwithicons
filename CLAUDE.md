...
**Giacomo Casanova → Arthur Vance swap (2026-09-07):** Casanova's Vapi assistant slot (routing key `sittingbull`, phone `+15854073097`) was repurposed for Arthur Vance — an independent UFO/UAP field researcher, fifty-two years of case files, never government, never academic. Unlike prior swaps, Vance's assistant was NOT built by repurposing Casanova's existing assistant record — a new assistant (`bc8ba6dd-a90f-41cc-aa08-ca208107f864`) was built and fully CANON'd ahead of time, then the `sittingbull` slot's routing (`VAPI_ASSISTANT_ID_SITTINGBULL` env var) was pointed at it. Casanova's old assistant (`101718d5-fc95-462c-8ca8-a966aefed7b4`) is left dormant, not deleted, per standard practice. Full cleanup done same day: kadevoss.html + kadevoss-feature.html rebuilt in place (filenames and `character: 'sittingbull'` unchanged in both), index.html roster card swapped (gold hook line: "The truth is out there. Whether you believe or not. You've seen the Navy videos. Now speak with a man who spent 50 years interviewing many that nobody believed."), all 22 sibling character/feature pages' cross-promo sections updated, api/call-ended.js ASSISTANT_NAMES map, and this file's Assistant IDs / ElevenLabs Voices tables. Real portrait supplied by Ruby (images/vance.jpeg, 1254×1254). Vance's Rule 2 turn-ending check-in phrases were deliberately written distinct from Walter Hobbs's (file/notes/case vocabulary rather than Hobbs's "thread"/"short version" language) despite following the same three-layer mechanic (Rule 12 above).

**Ching Shih phone number released (2026-09-22):** Ching Shih (routing key `brucelee`, assistant `0964cfaa-6059-4a55-b71f-1d8c74fe690f`) was confirmed by the user as not an active icon — no linked page exists, matching Silver and Friday's state, but unlike them she still had a live, dialable Twilio number: `+16293307964` (Vapi phone-number id `1a3c22e7-0b6e-4965-8207-450022f2147a`, Twilio SID `PN084640e405e54e2f834ace059457c555`, labeled "Bruce Lee" — a stale name left over from the original Bruce Lee → Zhou Han/Ching Shih swap). Checked whether Vincent's outbound calls used this number first (`VAPI_PHONE_NUMBER_ID_VINCENT` is unset in Vercel, so Vincent actually uses `VAPI_PHONE_NUMBER_ID_DEFAULT` = `f1715de0-2c12-462c-9f5b-5621bdf3e3ce`, a different number) — confirmed unused by Vincent, so per the Twain/La Llorona lesson the number was deleted outright rather than reassigned: full phone-number list backed up to `vapi-backup/phone-numbers-pre-cleanup.json` first, then deleted via Vapi (`DELETE /phone-number/{id}`, confirmed gone, 11 numbers remaining) and released in Twilio (`DELETE /IncomingPhoneNumbers/{sid}`, 204, confirmed gone from Twilio's own list). Ching Shih's assistant record itself was left dormant, not deleted, per standard practice. Checked Silver (`b98cec95-47a4-455d-92c8-3a08aacb556d`) and Friday (`b1f66c71-a2d6-4ae9-9ea1-7b72d4a423d0`) for the same leftover-number problem in the same pass — as of this date neither has any phone number attached in Vapi (not in the `/phone-number` list, no `phoneNumberId` field on either assistant record), so there was nothing to delete for them.

**Phone routing fixes, both closed out (2026-09-22):**

1. `f1715de0-2c12-462c-9f5b-5621bdf3e3ce` (`+19182767181`, `VAPI_PHONE_NUMBER_ID_DEFAULT`) was found still routing inbound to Evangeline while also being Vincent's silent fallback (his own `VAPI_PHONE_NUMBER_ID_VINCENT` was unset). First patched to Vincent as a stopgap — then superseded same day by giving Vincent his own dedicated number (below), so this one was **reverted back to Evangeline** (`assistantId` PATCHed to `7fd88fa7-f013-4693-9b52-ab8937e4225d`, backed up before each patch to `vapi-backup/phonenumber-f1715de0-pre-routing-fix.json` and `vapi-backup/phonenumber-f1715de0-pre-evangeline-revert.json`). `+19182767181` is Evangeline's alone again, no sharing.
2. `592fda30-7a31-49e7-b793-eb1f569a7d9c` (`+14632886610`, `sittingbull` routing key, still labeled "Friday" in Vapi) — was still routing inbound to the retired Casanova (`101718d5-fc95-462c-8ca8-a966aefed7b4`) despite the Casanova → Vance swap above being documented as complete. Rule 7: GET + backup (`vapi-backup/phonenumber-592fda30-pre-routing-fix.json`), PATCHed `assistantId` to Arthur Vance (`bc8ba6dd-a90f-41cc-aa08-ca208107f864`), GET-after confirmed. Casanova's assistant record left dormant, not deleted.
3. **Vincent given his own dedicated number instead of sharing Evangeline's.** Diffed the full Twilio `IncomingPhoneNumbers` list against Vapi's `/phone-number` list — exactly one Twilio number existed but was never imported into Vapi: `+14639464699`. That number was not a clean spare: it's `SYNTHETIC_CALL_TEST_NUMBER`, the destination `api/synthetic-call-check.js`'s daily cron dials from Tesla's assistant to catch pipeline failures (it works today specifically *because* nothing owns that number in Vapi, so the call rings into nothing and the check just reads the outbound leg's result). Importing it straight to Vincent would have made that daily cron connect to Vincent's live assistant instead — two AI characters on one call, real recurring cost, every morning, unprompted. Flagged this to the user before touching it. Fix: provisioned a free Vapi-hosted number (`POST /phone-number` with `provider: "vapi"`, no Twilio purchase — Vapi supports this, just needs `numberDesiredAreaCode`) with no assistant attached, `+14632150226` (id `81193bf9-acb8-409c-90de-0405842b6f36`), and repointed `SYNTHETIC_CALL_TEST_NUMBER` in Vercel to it. That freed `+14639464699` completely, which was then imported into Vapi (`provider: "twilio"`, Twilio SID/token, `assistantId` = Vincent) as `c52a0a8d-027e-41e2-bccf-af3b9c1fac91`, and `VAPI_PHONE_NUMBER_ID_VINCENT` set to that id. Same sensitive-default gotcha as before on both new env vars — both re-added with `--no-sensitive`, both verified to pull back with real values.

**Final number-to-icon mapping (2026-09-22, 12 Twilio numbers + 1 free Vapi number, for 12 live icons — no two icons share a number anymore):** Aela `+14784018828`, Céleste `+14632891250`, Da Vinci `+14632923593`, Evangeline `+19182767181`, Houdini `+15858448126`, Nostradamus `+14173873982`, Rocca `+14632514283`, Tesla `+15858445968`, Vincent van Gogh `+14639464699`, Walter Hobbs `+12407894866`, Watson `+14632595648`, Arthur Vance `+14632886610`. Separately, the daily synthetic-call-check destination is its own free Vapi number, `+14632150226`, decoupled from every character's number — see the next entry for what's now behind it.

**Synthetic health-check audited and fixed (2026-09-22):** `api/synthetic-call-check.js`'s daily cron (Tesla calls `SYNTHETIC_CALL_TEST_NUMBER`) previously only checked whether the final `endedReason` avoided Vapi's `pipeline-error-*` prefix — it never confirmed the call was answered, reached `in-progress`, or that any speech was ever produced. Since the test number has never had an assistant behind it, the call was never actually answered, so the check could report healthy even if Tesla's LLM/TTS never ran at all; the comment claiming it caught the 2026-08-06 ElevenLabs failure is plausible but wasn't actually verifying live speech. Fixed same day: created a minimal, non-character "Synthetic Test Receiver" assistant (id `6adcfc6c-31c5-4df7-86e9-876d05cf5964`, `gpt-4o-mini`, one fixed line, `maxDurationSeconds: 20`, `endCallPhrases: ["Goodbye."]` — hard-capped so it can't run long or cost meaningfully) and attached it to the free Vapi number `+14632150226`, so Tesla's daily call is now actually answered instead of ringing into nothing. The check itself now polls every 5s (up to the same 60s ceiling) instead of one fixed sleep-then-check, and a pass requires all three: `startedAt` is set (the call was genuinely answered — far more reliable than trying to catch a narrow `in-progress` status snapshot on a short call), at least one non-empty `role: "bot"` transcript message exists (proves the LLM generated a real turn that required TTS to produce), and `endedReason` still isn't `pipeline-error-*`. Verified end to end with a real call before shipping (DB/SMS alerting stubbed out for the test run only, to avoid polluting `health_check_state`): `wasAnswered: true`, `hasBotSpeech: true`, `endedReason: "customer-ended-call"`, `ok: true`.
...
| ~~Friday~~ | ~~b1f66c71-a2d6-4ae9-9ea1-7b72d4a423d0~~ | ~~+15854073097~~ (slot later became Kade Voss, then Giacomo Casanova, then Arthur Vance — routing key `sittingbull` never renamed across any of these) |
| ~~Giacomo Casanova~~ | ~~101718d5-fc95-462c-8ca8-a966aefed7b4~~ | +15854073097 (retired 2026-09-07, dormant not deleted in Vapi, replaced by Arthur Vance) |
| Arthur Vance | bc8ba6dd-a90f-41cc-aa08-ca208107f864 | +15854073097 (occupies the legacy `sittingbull` routing slot; env var `VAPI_ASSISTANT_ID_SITTINGBULL`) |
...
| Arthur Vance | Voice ID: IBZnCd46b501W5pcmF2O ("Vance 2") | Voice Design voice, purpose-built 2026-09; eleven_turbo_v2_5, stability 0.45, style 0.3, speed 0.92 |
...
### Rule 12: Turn-ending mechanic — offer more of your own material first; check in only once a thought has run its course; never inward

Every character's turn-ending mechanic (the thing that fires instead of ending in silence) has three layers, in this order of priority:

1. **Shorter exchanges** (most exchanges): no more than 3-4 sentences before checking whether the caller wants more. Match response depth to question depth.
2. **Deep-topic exception** (each character names its own 2-4 topics this applies to — cases/method/witnessed for Rocca, mission/Pleiades/human potential for Aela, painting/color/Theo/the illness for Vincent): a complete account up to ~200 words in one turn is allowed, no early check-in required. This is a per-character list of what actually deserves length, not a blanket allowance.
3. **After finishing a substantial account, the default close is to offer more of the character's own material** — another memory, what came before it, what happened next — **not** to immediately turn a question to the caller. Only once that offer's been made and not taken further does the character check in with a plain, mechanical line: "Does that interest you, or should I tell you something else?" / "Do you have another question?" / "Should I go on?" This check-in triggers when a thought has actually run its course, on no fixed schedule — not after every single chunk regardless of topic.

**If a character does ask something more than a floor-check**, it must point **outward** — an idea, opinion, belief, a piece of the character's own world the caller can answer freely — **never inward**, asking the caller to hand over their own history, sacrifice, fear, loss, or a time they tried and failed. That register isn't any character's to reach for uninvited. (Evangeline is the one remaining documented exception with a real in-character reason to ask inward — don't "fix" her if audited. Aela was also an exception until 2026-09-22 — see below, that permission is now revoked.)

**Aela-specific exception (2026-09-22): caller-question permission fully revoked, roster-wide standard superseded by a stricter one for her alone.** Aela no longer asks {{callerName}} any question about themselves — not outward, not inward, not a floor-check, not a permission-ask. Every turn-ending mechanic in her prompt (Rule 3's turn-ending/question layer, the real Rule 9's "ask about what the caller said" mandate, all nine Beat 1/Beat 2 knowledge-topic check-ins, and the four caller-directed example questions in HOW YOU LISTEN) was replaced with a single canonical continuation-offer rule ("HOW YOU END A TURN") inserted into Rule 3: she names the specific topic she was just on and offers either more of it or lets the caller ask something else — never a question aimed at the caller's own life, beliefs, or feelings. A floating, unheadered rule block that predated this fix (a "2nd/4th initiated question" pacing hack, numerically colliding with the real Rule 9 — a duplicate-rule artifact like the old search-tool duplication) was deleted outright rather than converted, since it existed only to pace caller-questions that no longer happen. The 120-word ceiling, the 200-word narrative-carve-out allowance, the punctuation-pacing rule, search_web instructions, and firstMessage were all left untouched — verified byte-identical against a pre-patch backup. Patch script: `patch-aela-no-questions.js`. Backup: `vapi-backup-response-limit/aela-pre-noquestions.json`. **Do not roll Aela back into the roster-wide Rule 12 pattern (outward-questions-allowed) without being asked — her current behavior is intentionally stricter than the rest of the roster.**

**Why documented this way:** the outward/inward half was already a real, applied roster-wide standard (rolled out via `patch-roster-question-fix-standard.js` to Rocca, Nostradamus, Aela, Céleste, Walter Hobbs, Zhou Han/Ching Shih, Watson, Tesla, Casanova) but was never written into this file. Building Vincent from scratch (2026-09-01) surfaced two successive misses because of that: first his turn-ending rule had no outward/inward constraint at all (fixed same-day after a real test call showed inward drift); then the fix for *that* made floor-checking the default close instead of the actual established pattern — offer-more-first, floor-check only once a thought is spent — which produced a mechanical "asks to continue every 4 sentences" problem on a second real test call (also fixed same-day, by finally reading Aela's actual live Rule 3 in full rather than reconstructing the pattern from a partial read of Rocca's). New characters must include all three layers in their turn-ending rule from creation (Rule 9), with their own deep-topic list for layer 2 — don't reinvent this per-character from a vague instinct; copy Aela's or Rocca's actual live prompt text for the pattern, not a paraphrase of it.

### Rule 13: 1963 Period Lock — mandatory for every generated still in the Walter Hobbs reel series

Every element in frame must be explicitly period-correct 1963 Dallas — nothing left to the image generator's defaults. Apply all of the following in the generation prompt itself, every time:

- **People:** 1963 dress only. Men: dark suit or sport coat, white shirt, narrow tie, or shirtsleeves with fedora/trilby; leather dress shoes. Women: below-knee dresses or skirt suits, pillbox hat or headscarf. Never sneakers, tracksuits, jeans, t-shirts, backpacks, or sunglasses styles from after 1963.
- **Setting:** state the real geography explicitly — Dealey Plaza, Dallas: open grassy slopes beside Elm Street, white concrete pergola and colonnade, the brick Texas School Book Depository building in the background where relevant. Never "a street" or "an avenue" generically — name the real place.
- **Vehicles, if any:** early-1960s American sedans only. No modern cars, signage, road markings, or street furniture.
- **Negative instructions belong in the prompt itself**, not just implied: "no modern clothing, no sneakers, no modern cars, no modern signage."
- **Pass/fail check before any Runway motion pass:** would every object in this frame plausibly exist in Dallas in 1963? If not, regenerate the still — never try to fix it in the motion pass.

**Why:** the first "Umbrella Man" pass generated a figure in modern joggers and sneakers on a vacant generic street — no period cars, no real Dealey Plaza geography, no crowd. It looked contemporary because nothing in the prompt locked it to 1963 specifically; the generator filled every unconstrained detail with its own modern default. The fix that actually worked was supplying a real reference photo (`images/walter-hobbs-young-elm-st.jpeg`, an existing site asset already showing real Dealey Plaza geometry, period dress, and a period crowd) as a Runway `referenceImages` input, plus stating the real place name and period details explicitly in the prompt rather than trusting generic "1963" or "period" language alone. Every future still in this series must pass this checklist before spending a Runway video credit on it.

### Rule 14: Mandatory pre-flight contradiction check — every Runway prompt, before any credits are spent

Before submitting ANY image or motion prompt to Runway (any reel series, any character), read the prompt back against itself and verify no instruction contradicts another. This is a required step, not optional judgment — do it every time, out loud in the response if useful, before the API call.

**Specifically check for:**
1. **A named physical object, light source, or element** (e.g. "candlelight flicker," "dust drifting," "curtain moving," "a lamp glowing") **stated in the same prompt as a constraint forbidding new/added elements** (e.g. "nothing added to the frame," "no objects added," "no new people"). Naming a thing and forbidding new things in the same breath is a direct contradiction — the model resolves it by inventing the named thing anyway.
2. **An identity/face lock stated only for the opening** ("stays locked at the start," "identity locked" with no duration scope) **rather than covering the full clip explicitly.** Required phrasing: identity/face locked "at any point, from first frame to last" (or equivalent full-duration language) — every time, on every prompt with a real person's face in frame.
3. **Any motion instruction that requires inventing new geometry** — a light source not in the source photo, an object not in frame — **rather than animating what's actually there**: camera movement, existing light/shadow shift, atmospheric grain/dust already implied by the photo's age, environment already in frame moving.

**If a contradiction is found, rewrite before submission:** describe motion only through camera movement, existing light/shadow shift, and atmosphere — never by naming a new object or light source not already present in the source image. State identity/face lock as covering the entire clip duration explicitly, every time a real person's face is in frame.

**Why:** Houdini Reel 5's clip2 prompt asked for "candlelight-style flicker" in the same sentence as "nothing added to the frame" — the model took the candlelight instruction literally, hallucinated an actual candle into the shot, and identity coherence broke down alongside it (both Houdini and Bess drifted into generic AI faces by ~6s into a 9s clip). This wasn't caught before shipping because verification only checked the opening 1-2 seconds, not the full clip (see Rule verification practice in [[project_talkwithicons_houdini_reel_series]] via the assistant's memory system). Cost: 108 credits to reshoot, on an account later found to be down to ~565 real remaining API credits with real autopay now attached — a contradiction like this is not a free mistake anymore. This check costs zero credits and takes one read-through; there is no reason to ever skip it.

### Rule 15: Source photo resolution floor — check native resolution before every Runway submission, alongside Rule 14

Before submitting any real photograph to Runway for a motion clip involving a named person's likeness, check the source image's **native resolution** before generation — not just its authenticity and period-accuracy (which Rule 14 and the standard image-inspection checklist already cover). This is a separate check, done every time, alongside Rule 14's contradiction check.

**Practical guideline (refine with more data over time):** treat any source photo under roughly **800px on its shorter dimension** as high risk for full-duration identity drift on any clip longer than about 5 seconds.

**For any such low-resolution source:**
1. Prefer a higher-resolution alternative first, if one exists and can be verified as equally authentic and public domain.
2. If no better source exists, default the clip to a **shorter duration (5s or less)** and use freeze-extend to reach the needed length, rather than generating a longer clip and hoping identity holds. This is cheaper than a failed long-duration attempt.
3. State the source image's resolution explicitly in every report going forward, the same way period/authenticity is already reported — a visible check, not an assumption.

**Why:** in the Walter Hobbs "I Don't Either" reel, a 430×640 (further cropped) photo of Senator Richard Russell held identity for roughly 2-3 seconds before visible drift began, and had fully lost likeness by 9 seconds — while a higher-resolution LBJ photo used in the same reel held cleanly through its full duration with zero drift. The failure traced directly to source resolution, not prompt quality — the identity-lock language was already at the strongest phrasing established by Rule 14. The fix that worked: resubmit at 5s (a duration the low-res source could actually hold), verify it stayed clean end to end, then freeze-extend the last clean frame to reach the needed length. Cost of not catching this before generation: 108 credits burned on the failed 9s attempt before the shorter-duration fix was found. Checking resolution before submission is free; discovering the problem after generation is not.

### Rule 16: Reel production, any character — read `REEL_PRODUCTION.md` before touching any reel work

That file is the full standing reference for the reel pipeline shared across every character, not just whichever one it was written during: Runway API mechanics and cost rules (including the mandatory pre-spend cost flag — sum the whole reel's credits and confirm with the user before submitting if it clearly exceeds the ~60-120 credit per-reel norm, even when the user's own brief specified the durations that produce the higher total), pre-flight still checks (resolution, aspect-ratio-vs-crop, real-person likeness, watermarks), motion-prompt discipline, post-generation verification, assembly gotchas (faststart, SAR mismatches, the multi-gblur concat-desync bug and its fix, attribution-vs-caption-wrap collisions), the shared end-card structure, CANON/Vapi-prompt editing procedure (works the same way for any character's assistant), and site-copy discipline. Do not rebuild any of this from scratch or relearn it by repeating a past mistake on a different character — it's all already written down.

Every reel, for every character, also gets a spec file at `reel-specs/<character>-reel-<name>.md` — see that directory for the format.
