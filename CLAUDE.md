...
### Rule 16: Arthur Vance reel production — read `VANCE_REEL_PRODUCTION.md` before touching any reel work for him

That file is the full standing reference for this character's reel pipeline: Runway API mechanics and cost rules (including the mandatory pre-spend cost flag — sum the whole reel's credits and confirm with the user before submitting if it clearly exceeds the ~60-120 credit per-reel norm, even when the user's own brief specified the durations that produce the higher total), pre-flight still checks (resolution, aspect-ratio-vs-crop, real-person likeness, watermarks), motion-prompt discipline, post-generation verification, assembly gotchas (faststart, SAR mismatches, the multi-gblur concat-desync bug and its fix, attribution-vs-caption-wrap collisions), the house end-card format, CANON/Vapi-prompt editing procedure, and site-copy discipline. Do not rebuild any of this from scratch or relearn it by repeating a past mistake — it's all already written down.

Every reel also gets a spec file at `reel-specs/vance-reel-<name>.md` — see that directory for the format.
