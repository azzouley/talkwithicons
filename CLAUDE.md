...
1. `f1715de0-2c12-462c-9f5b-5621bdf3e3ce` (`+19182767181`) — this is `VAPI_PHONE_NUMBER_ID_DEFAULT`, and was found still routing inbound to Evangeline (its original owner) while also being Vincent's silent fallback (his own `VAPI_PHONE_NUMBER_ID_VINCENT` was unset). Rule 7: GET + backup (`vapi-backup/phonenumber-f1715de0-pre-routing-fix.json`) before PATCH. `assistantId` PATCHed from Evangeline (`7fd88fa7-...`) to Vincent (`ae3e744a-ebb7-47f8-bfdb-35093f66f4b0`), GET-after confirmed. `VAPI_PHONE_NUMBER_ID_VINCENT` then set explicitly in Vercel Production to `f1715de0-2c12-462c-9f5b-5621bdf3e3ce`, so he no longer depends on the fallback. **Gotcha hit while setting it:** `vercel env add` on Production defaults to `sensitive`, which saves the value correctly but makes `vercel env pull` return `""` for it (same failure mode already documented under [[feedback_vercel_env_add_sensitive_default]] in memory) — fixed by removing and re-adding with `--no-sensitive --value "..."`, confirmed the real value pulls back correctly. A fresh git-sourced deploy (not a deploymentId redeploy) was required to pick it up — the commit for this change served as that deploy. **Evangeline still outbound-shares this same physical number** — her own flow (`api/start-call.js`) reads `VAPI_PHONE_NUMBER_ID` (the same default) directly as her caller-ID number, independent of the number's own inbound-routing `assistantId`. So `+19182767181` is now Vincent's inbound number and remains Evangeline's outbound caller ID; if it's ever dialed back, it reaches Vincent, not Evangeline.
...
**Final number-to-icon mapping (2026-09-22, 11 Twilio numbers for 12 live icons):** Aela `+14784018828`, Céleste `+14632891250`, Da Vinci `+14632923593`, Houdini `+15858448126`, Nostradamus `+14173873982`, Rocca `+14632514283`, Tesla `+15858445968`, Vincent van Gogh `+19182767181`, Walter Hobbs `+12407894866`, Watson `+14632595648`, Arthur Vance `+14632886610`. **Evangeline has no dedicated number** — she shares Vincent's `+19182767181` for outbound caller ID only (see above), which is the one pair of icons sharing a number and the reason the count is 11 rather than 12.
